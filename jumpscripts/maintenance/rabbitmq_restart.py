    ...
    if OpenvStorageHealthCheck.LOCAL_SR.node_type == "MASTER":
        ...
        if cls_status[0] != 200:  # in case of cluster_status as error
            j.errorconditionhandler.raiseOperationalWarning(
                category=category,
                tags="RabbitMQ",
                message="Couldn't retrieve RabbitMQ cluster status",
            )
            return
        else:
            ...
            if len(cls_status[1][hostname]["partitions"]) > 1:
                ...
                j.errorconditionhandler.raiseOperationalWarning(
                    category=category,
                    tags="RabbitMQ",
                    message="RabbitMQ on {} has partitions issues will restart to solve.".format(
                        hostname
                    ),
                )
                j.system.process.execute(
                    """rabbitmqctl stop_app; rabbitmqctl reset; rabbitmqctl join_cluster {}; 
                                            rabbitmqctl start_app; systemctl restart ovs-workers""".format(
                        node_in_cluster
                    )
                )
