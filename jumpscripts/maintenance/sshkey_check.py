...
def action():
    ...
    if not j.system.fs.exists("/root/.ssh/id_rsa"):
        j.system.platform.ubuntu.generateLocalSSHKeyPair(
            passphrase="", type="rsa", overwrite=True, path="/root/.ssh/id_rsa"
        )
    ...
    if j.system.fs.exists("/etc/ssh/ssh_host_rsa_key.pub"):
        ...
        print(
            "Current node doesn't have host key file at /etc/ssh/ssh_host_rsa_key.pub"
        )
    # save node to db
    ...
    if j.system.fs.exists("/root/.ssh/authorized_keys"):
        ...
        j.errorconditionhandler.raiseOperationalWarning(
            message="checks nodes for sshkey from other nodes are authorized on nid:%s and gid:%s "
            % (nid, gid),
