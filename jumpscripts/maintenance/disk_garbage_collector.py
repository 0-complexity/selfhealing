    ...
    for vpool in VPoolList.get_vpools():
        ...
        config_file = Configuration.get_configuration_path(
            "/ovs/vpools/{0}/hosts/{1}/config".format(
                vpool.guid, vpool.storagedrivers[0].name
            )
        )
        ...
        arakoon_cluster_id = str(
            config["volume_registry"]["vregistry_arakoon_cluster_id"]
        )
        arakoon_cluster_nodes = [
            src.ArakoonNodeConfig(
                str(node["node_id"]), str(node["host"]), int(node["port"])
            )
            for node in config["volume_registry"]["vregistry_arakoon_cluster_nodes"]
        ]

        object_registry_client = src.ObjectRegistryClient(
            cluster_id, arakoon_cluster_id, arakoon_cluster_nodes
        )
        filesystem_metadata_client = src.FileSystemMetaDataClient(
            cluster_id, arakoon_cluster_id, arakoon_cluster_nodes
        )
        cluster_registery = src.ClusterRegistry(
            cluster_id, arakoon_cluster_id, arakoon_cluster_nodes
        )
        ...
        for volume_id, volume_info in volume_locations.iteritems():
            try:
                heal_volume(
                    ...
                    object_registry_client,
                    filesystem_metadata_client,
                    cluster_registery,
                    vpool.guid,
                    mountpoint,
                    osis,
                ...
                print "Got {0} while removing. Metadata might not stale for {1}. Remove it using the GUI or API.".format(
                    str(ex), volume_id
                )
    ...
    def do(start_path):
        ...
        for entry in os.listdir(start_path):
            ...
            if os.path.isdir(entry_path):
                ...
                object_id = filesystem_metadata_client.find_path(
                    entry_path[len(mountpoint) :]
                ).object_id()
                volumes[object_id] = {
                    "dir": "{0}/".format(start_path),
                    "volume_name": entry,
                }
        return volumes
