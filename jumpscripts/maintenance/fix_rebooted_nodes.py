...
def action():
    ...
    if result:
        ...
        raise RuntimeError(
            "This node id does not belong to a cpu node please check where you are running this script"
        )
    # get all running virtual machines on this node
    model_virtual_machines = ccl.vmachine.search(
        {"status": "RUNNING", "stackId": stack_id}
    )[1:]
    time_limit = j.base.time.getTimeEpoch() - 300
    for machine in ccl.vmachine.search({"status": "HALTED", "stackId": stack_id})[1:]:
        results = scl.audit.search(
            {
                "tags": {"$regex": ".*machineId:%s.*" % machine["id"]},
    ...
    all_virtual_firewalls = vcl.virtualfirewall.search(
        {"nid": nid, "type": "routeros"}
    )[1:]
    ...
    for machine_model in model_virtual_machines:
        if machine_model["referenceId"] in domains_uuid:
            continue
        pcl.actors.cloudbroker.machine.start(
            machineId=machine_model["id"], reason="node reboot"
        )
    ...
    for vfw in all_virtual_firewalls:
        ...
        if not cloudspace:
            j.errorcondtiion.raiseOperationWarning(
                "no cloudspace with networkId %s available" % vfw["networkid"]
            )
