...
def action():
    ...
    stacks = ccl.stack.search(
        {"status": {"$in": ["ENABLED", "ERROR"]}, "gid": j.application.whoAmI.gid}
    )[1:]
    connection_ok = True
    for stack in stacks:
        for key, value in sessions.iteritems():
            if int(key.split("_")[1]) == int(stack["referenceId"]):
                ...
                if ti > 120:
                    ...
                    try:
                        sshclient = j.remote.ssh.getSSHClient(
                    ...
                    if not connection_ok or exit_code != 0:
                        pcl.actors.cloudbroker.node.maintenance(
                            nid=stack["referenceId"], vmaction="move"
                        )
                        msg = "Node: %s is put in maintenance mode" % (
                            stack["referenceId"]
                        )
                    else:
                        msg = "Jsagent for node: %s has been restarted" % (
                            stack["referenceId"]
                        )
