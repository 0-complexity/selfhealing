    ...
    for cs in ccl.cloudspace.search({"status": {"$in": ["DEPLOYING", "VIRTUAL"]}})[1:]:
        ...
        if cs["status"] == "DEPLOYING":
            if vcl.virtualfirewall.exists(vfwid):
                ...
                job = next(
                    iter(scl.job.search({"guid": vfw.deployment_jobguid})[1:]), None
                )
        ...
        if redeploy:
            if (
                ccl.account.search({"id": cs["accountId"], "status": "CONFIRMED"})[0]
                != 1
            ):
                j.console.warning(
                    "Could not find confirmed account for cloudspace {id} {name}".format(
                        **cs
                    )
                )
                continue
            if cs["status"] == "DEPLOYING":
                ccl.cloudspace.updateSearch(
                    {"id": cs["id"]}, {"$set": {"status": "VIRTUAL"}}
                )
            ...
            try:
                ...
                j.errorconditionhandler.raiseOperationalWarning(
                    message="deploy cloudspace %s on nid:%s gid:%s"
                    % (cs["name"], nid, gid),
