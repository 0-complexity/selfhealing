...
def action():
    ...
    if j.system.fs.exists("/etc/ssh/ssh_host_rsa_key.pub"):
        ...
        print "node %s doesn't have host key file at /etc/ssh/ssh_host_rsa_key.pub" % current_node[
            "name"
        ]
    # save node to db
    ...
    if j.system.fs.exists(authorizedfile):
        ...
        j.errorconditionhandler.raiseOperationalWarning(
            message="reflector add sshkeys to authorized file on nid:%s and gid:%s "
            % (nid, gid),
    ...
    changes = {"public": False, "host": False}
    for node in nodes:
        # deal with public keys
        if len(node["publickeys"]) <= 0:
            print "node %s doesn't have keys from node %s" % (
                current_node["name"],
                node["name"],
            )
        else:
            ...
            print "node %s have keys from node %s" % (
                current_node["name"],
                node["name"],
            )
