...
def calc_network_data(interface_name, interface_type="interface"):
    ...
    if interface_type == "routeros":
        result['network.vfw.throughput.outgoing'] = int(round(bytes_sent / (1024.0 * 1024), 0))
        result['network.vfw.throughput.incoming'] = int(round(bytes_recv / (1024.0 * 1024), 0))
        ...
        if vm is None:
            ...
            for interface_name in interfaces_names:
                ...
                for key, value in result.iteritems():
                    ...
                    tags = 'gid:%d nid:%d nic:%s type:virtual' % (j.application.whoAmI.gid, j.application.whoAmI.nid, interface_name)
