...
def action():
    ...
    for path, counter in counters.iteritems():
        ...
        if partition is not None:
            ...
            results["space_percent"] = int(
                round((float(usage.used) / float(usage.total)), 2)
            )
        ...
        stats = {
            "disk.iops.read": {"value_key": "count_read", "type": "D"},
            "disk.iops.write": {"value_key": "count_write", "type": "D"},
            "disk.throughput.read": {"value_key": "kbytes_read", "type": "D"},
            "disk.throughput.write": {"value_key": "kbytes_write", "type": "D"},
        }
        for key, info in stats.iteritems():
            ...
            tags = "gid:%d nid:%d device:%s type:physical" % (
                j.application.whoAmI.gid,
                j.application.whoAmI.nid,
                path,
            )
