...
def action():
    ...
    for f in labeled:
        ...
        if "core" in label.lower():
            ...
            key = "machine.CPU.temperature@phys.{gid}.{nid}.{coreid}".format(
                gid=gid, nid=nid, coreid=coreid
            )
            tags = "gid:%d nid:%d labelfile:%s" % (
    ...
    def disktemp(disk):
        ...
        rc, out = j.system.process.execute(
            cmd, dieOnNonZeroExitCode=False, noDuplicates=True
        )
        if rc == 0 and out:
            for match in re.finditer(pat, out, re.M):
                current = int(match.group("temp"))
                print("Disk {} at {}".format(disk, current))
                key = "machine.disk.temperature@phys.{gid}.{nid}.{disk}".format(
                    gid=gid, nid=nid, disk=disk
                )
                tags = "gid:%d nid:%d" % (
                    j.application.whoAmI.gid,
                    j.application.whoAmI.nid,
    ...
    disks = (
        ...
        + glob.glob("/sys/block/hd*")
        + glob.glob("/sys/block/nvm*")
    )
