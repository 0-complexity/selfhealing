    ...
    for cloudspace in cloudspaces:
        ...
        for vm in vms:
            ...
            vm_data = {
                ...
                "ping": False,
                "hdtest": False,
                "cpu_node_name": cpu_node_name,
                "cpu_node_id": cpu_node_id,
            ...
            if vm["status"] == "RUNNING":
                ...
                if ipaddress != "Undefined" and vfw:
                    args = {
                        ...
                        "vm_cloudspace_id": cloudspace["id"],
                    }
                    job = accl.scheduleCmd(
                        vfw.gid,
                        vfw.nid,
                        "jumpscale",
                        "vm_ping",
            ...
            if vm["status"] and cpu_node_id:
                diskpaths = [
                    disk["referenceId"]
                    for disk in cbcl.disk.search({"id": {"$in": vm["disks"]}})[1:]
                ]
                job = accl.scheduleCmd(
                    gid,
                    cpu_node_id,
