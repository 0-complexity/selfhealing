...
def action():
    category = "Hardware"
    rc, output = j.system.process.execute(
        "ovs-appctl bond/show", dieOnNonZeroExitCode=False, noDuplicates=True
    )
    results = []
    uid = "ovc-appctl"
    if rc == 127:
        ...
        results.append(
            {"message": msg, "uid": uid, "category": category, "state": "ERROR"}
        )
    else:
        bonds = []
        bond = {}
        ...
        for bond in bonds:
            ...
            state = "OK"
            if badslaves:
                msg = "Bond: {} has problems with slaves {}".format(
                    bond["name"], ", ".join(badslaves)
                )
