    ...
    for proc in psutil.get_process_list():
        try:
            ...
            if state is not None:
                msg = "Too many open file descriptors for {} with PID {} {}/{}".format(
                    proc.cmdline(), proc.pid, count, soft
                )
                uid = "openfd:{}".format(proc.pid)
                results.append(
                    {"state": state, "message": msg, "category": category, "uid": uid}
                )
