...
def action():
    ...
    for stat in statsclient.statsByPrefix("network.throughput"):
        ...
        try:
            nicspeed = (
                j.system.net.getNicSpeed(pnic) / 8.
            )  # nic speed is expressed in mbit but we want mbytes
        except:
            continue

        percent = ((stat.m_last / 300.) / float(nicspeed)) * 100
        message["message"] = "Nic {} {} bandwith is {:.2f}%".format(
            nic, direction, percent
        )
        ...
        results.append(
            {
                "state": "OK",
                "category": "Network",
                "message": "All network bandwith is within boundries",
            }
        )
    return results
