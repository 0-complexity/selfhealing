...
def check_over_memory(value, reasons):
    ...
    if percent >= MEMORY_THRESHOLD:
        ...
        reasons.messages.append(
            "{}/{} memory".format(percent * 100, MEMORY_THRESHOLD * 100)
        )
        return True
    return False
...
def check_volume_read(ovscl, driver, reasons):
    ...
    driver_obj = ovscl.get(
        "/storagedrivers/{}".format(driver["guid"]),
        {"contents": "cluster_node_config,vdisks_guids"},
    )
    ...
    for vdisk in disks:
        ...
        try:
            ...
            if code != 0:
                reasons.messages.append(
                    "Failed to read volume {} ({})".format(
                        vdisk["devicename"], vdisk["guid"]
                    )
                )
            ...
            reasons.messages.append(
                "Reading of volume timedout {} ({})".format(
                    vdisk["devicename"], vdisk["guid"]
                )
            )
...
def get_memory_avg(agg, pool):
    key = "process.memory.vmrss@phys.%d.%d.%s_%s" % (
...
def get_vpool(ps):
    ...
    if m is None:
        raise RuntimeError(
            "cannot find the pool mount point for volumedriver pid: %s", ps.pid
        )
...
def is_active(vpool):
    active = j.system.platform.ubuntu.statusService("ovs-volumedriver_{}".format(vpool))
...
def action():
    ...
    for process in psutil.process_iter():
        try:
            ...
            if reasons.messages:
                message = "\n".join(
                    ["Volumedriver {} has some problems:".format(vpool)]
                    + reasons.messages
                )
                msg = {
