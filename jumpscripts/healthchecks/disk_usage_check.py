...
def action():
    ...
    def disktoStr(partition, usage):
        ...
        return "%s on %s %.02f/%.02f %siB free" % (
            partition.device,
    ...
    for partition in filter(diskfilter, psutil.disk_partitions()):
        ...
        checkusage = not (
            partition.mountpoint
            and j.system.fs.exists(
                j.system.fs.joinPaths(partition.mountpoint, ".dontreportusage")
            )
            or is_cache
        )
        result["state"] = "OK"
        ...
        if usage is not None:
            ...
            if (
                checkusage
                and tresholds.truncate > 0
                and (freepercent < tresholds.truncate)
            ):
                jumpscript = j.clients.redisworker.getJumpscriptFromName(
                    "0-complexity", "logs_truncate"
                )
                j.clients.redisworker.execJumpscript(
                    jumpscript=jumpscript, freespace_needed=40.0
                )
    ...
    if not results:
        results.append(
            {"message": "No disks available", "state": "OK", "category": "Disks"}
        )
