            ...
            for line in out.splitlines():
                if "|" in line:
                    ...
                    if sensorstatus == "ns" and "no reading" in message.lower():
                        results.append(dict(state='SKIPPED', category=category, uid=id_, message="Fan %s has no reading (%s)" % (id_, message)))
                    elif sensorstatus != "ok" and "no reading" not in message.lower():
                        results.append(dict(state='WARNING', category=category, uid=id_, message="Fan %s has problem (%s)" % (id_, message)))
            if len(results) == 0:
